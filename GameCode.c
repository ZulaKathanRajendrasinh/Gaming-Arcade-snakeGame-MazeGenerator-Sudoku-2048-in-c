...
char snakekey;
int la=3;
int gameover2048=1,gameovermaze=1;
...
//COUNT NUMBERS OF ERROR BY COMPARTING THE ARRAY
...
void solution(){
    ...
    for(i=1;i<=lenx;i++){
                printf(ANSI_COLOR_BRIGHT_GREEN"%c"ANSI_COLOR_RESET,bx);
            }
            printf("\n");
    ...
    for( j=2;j<=leny-1;j++){
        ...
        for( k=1;k<=lenx;k++){
            if(k%2==0){
                ...
                printf(ANSI_COLOR_BRIGHT_GREEN"%c"ANSI_COLOR_RESET,by);
            }
        }
        printf("\n");
        }
...
//RESHUFFLES THE EXISTING DOUBLE POINTER ARRAY
...
//GENERATE SPACE ON THE BASIS OF DIFFICULTY
...
void boundary(){
    system("cls");
    printf(ANSI_COLOR_RED"\n\t\t\t\tPress 'A' For Left,'D' For Right,'W' For Up,'S' For Down.\n");
        ...
        for( k=1;k<=lenx;k++){
            if(k%2==0){
                ...
                if(k==blockx && j==blocky){
                    if(arr[j][k]!='_'){
                        printf(ANSI_COLOR_BRIGHT_BLUE"%c"ANSI_COLOR_RESET,arr[j][k]);
                    
                    }
                    else if(arr[j][k]=='_'){
                       printf(ANSI_COLOR_RED"%c"ANSI_COLOR_RESET,point);     
                    }
                }
                else{
                printf("%c",arr[j][k]);
...
//MOVES THE POINTER TO THE SPACES 
void logic(){
...
void sudokugame(){
    ...
    err(ques,ptr);
    printf(ANSI_COLOR_BRIGHT_YELLOW"\nNo. Of Errors:%d"ANSI_COLOR_RESET,error);
    free(ptr);
    free(ques);
    Sleep(3000);
}
...
void play(int arr[4][4]){
    ...
    printf("Press X To Exit and Then Movement Key!");
    char key;
    int validInput = 0; 

    do {
        key=getch();
        if (key == 'W' || key == 'w' || key == 'S' || key == 's' || key=='x'||key=='X'||
...
//TO GENERATE RANDOM 4 NUMBERS 
int sideselection() {
    srand(time(NULL));
    return 1+(rand()%4);
}

//TO PRINT THE MAZE GENERATED FIRST TIME 
...
//TO GENERATE MAZE WHILE PLAYING GAME
...
//ALGORITHM FOR CHECKING UNOCCUPIED SPACES 
...
//TO MOVE THE B TO LEFT POSITION
...
//WHEN B REACHES F THIS CONDITION RETURNS TRUE
...
//TO PLAY THE MAZE MOVING THE B 
int playmaze(int size, char maze[size][size]) {
    printf(ANSI_COLOR_RED">* CONTROLS\n");
    printf(" FOR MOVES:- 'W','S','A','D'\n"ANSI_COLOR_RESET);
    printf(ANSI_COLOR_BRIGHT_YELLOW"Press X To Exist!\n"ANSI_COLOR_RESET);
    char key;
    int validInput = 0;
        key=getch();
...
//TO MAKE CELL SEPRATED THROUGH WALLS 
...
//TO GENERATE TOTALLY UNOCCUPIED MAZE 
...
//THIS FUNCTION OPERATES BACKTRACKING AND PRINT MAZE FUNCTION 
...
//THIS IS THE MAIN FUNCTION FOR CALLING OTHER FUNCTION 
void gamemaze(){
    system("cls");
    int size;
    ...
    while(1) {
        if(gameovermaze==0){
            printf(ANSI_COLOR_MAGENTA"Gameover!! Try again"ANSI_COLOR_RESET);
        return ;
     }
...
//buffer is used here to store the last snakepiece 
//every snakepiece is updated to the next piece 
...
//kbhit is a funtion in conio
//is press anything then kbhit changes or goes into the loop
...
// TO CHECK IF THE SNAKE HAS EATEN THE FOOD ELEMENT 
int eatfood(){
    int check = 0;
...
//IF EATFOOD RETURNS TRUE IT INCREASE THE SIZE OF THE SNAKE
void increaselen(){
...
//CHECKS IF THE SNAKE HAS HIT ITSELF OR THE WALL
void gameover(){
...
void snakegame(){
    ...
    for(move;snakegames == 1;move++){
        system("cls");
        printf(ANSI_COLOR_MAGENTA"                                                   <..SNAKE GAME..>\n\n\n"ANSI_COLOR_RESET);
        fillboard();
...
//THIS IS THE MAIN FUNCTION USING SWITCH WE CAN PLAY DIFFERENT GAMES 
int main(){
    char h='0';
